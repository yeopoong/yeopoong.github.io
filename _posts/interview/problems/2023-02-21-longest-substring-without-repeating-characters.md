...
tags: problems string sliding-window hashmap longest
---
    ...
    public int lengthOfLongestSubstring(String s) {
        ...
        while (end < s.length()) {
            ...
            if (!set.contains(s.charAt(end))) {
                // 처음 나온 문자면 저장하고 다음 문자로 간다.
                set.add(s.charAt(end++));
                // 최대 문자열 길이 갱신(윈도우 크기가 줄어들수도 있기 때문에 최대값을 계속 갱신해야 한다)
                max = Math.max(max, set.size());
            } else {
                // 이미 나온 문자면 저장소에서 해당 문자 삭제하고 시작 위치 갱신(종료 위치는 그대로 다음에 또 체크): 서브스트링을 구해야 하기 때문에 시작 윈도우 값을 증가하면서 중복된 문자를 찾는다.
